...
function render() {
  ...
  root.innerHTML = `
    <style>${styles()}</style>
    <div class="lumina-shell" style="transform: translate(${toolbarPosition.x}px, ${toolbarPosition.y}px)">
      <div class="lumina-grip" title="Drag toolbar" aria-label="Drag toolbar">
        <span></span><span></span><span></span><span></span><span></span><span></span>
      </div>
      <div class="lumina-live"><i></i>${state.status === RECORDING_STATUS.PAUSED ? 'Paused' : 'Recording'}</div>
      <div class="lumina-timer" data-timer>00:00</div>
      <button class="lumina-btn" data-action="${state.status === RECORDING_STATUS.PAUSED ? 'resume' : 'pause'}" title="${state.status === RECORDING_STATUS.PAUSED ? 'Resume' : 'Pause'}">
        ${state.status === RECORDING_STATUS.PAUSED ? playIcon() : pauseIcon()}
      </button>
      <button class="lumina-btn ${state.micEnabled ? '' : 'is-off'}" data-action="toggleMic" title="${state.micEnabled ? 'Mute microphone' : 'Unmute microphone'}">
        ${micIcon()}
      </button>
      <button class="lumina-btn ${state.cameraEnabled ? '' : 'is-off'}" data-action="toggleCamera" title="${state.cameraEnabled ? 'Hide camera' : 'Show camera'}">
        ${cameraIcon()}
      </button>
      <button class="lumina-stop" data-action="stop" title="Stop recording">${stopIcon()}</button>
    </div>
    <div class="lumina-camera" style="left:${Math.round(cameraPosition.x * window.innerWidth)}px; top:${Math.round(cameraPosition.y * window.innerHeight)}px; width:${cameraBubbleSize()}px; height:${cameraBubbleSize()}px">
      ${state.cameraEnabled ? `<iframe class="lumina-camera-frame" src="${chrome.runtime.getURL('camera/index.html')}" allow="camera"></iframe>` : `<div class="lumina-camera-placeholder">${avatarIcon()}</div>`}
...
function bindToolbar(root) {
  ...
  makeDraggable(root.querySelector('.lumina-camera'), {
    onMove: (x, y, element) => {
      ...
      cameraPosition = {
        ...cameraPosition,
        x: clamp(x / window.innerWidth, 0, 0.92),
        y: clamp(y / window.innerHeight, 0, 0.86)
      };
      chrome.runtime.sendMessage({
        type: MESSAGE.TOOLBAR_ACTION,
        action: 'cameraPosition',
        payload: {
          position: {
            x: clamp((x + rect.width / 2) / window.innerWidth, 0, 1),
            y: clamp((y + rect.height / 2) / window.innerHeight, 0, 1),
...
function styles() {
  ...
  return `
    ...
    .lumina-camera{position:absolute;overflow:hidden;border-radius:999px;background:linear-gradient(135deg,#111827,#312e81);box-shadow:0 18px 54px rgba(15,23,42,.28);pointer-events:auto;cursor:grab;animation:luminaBubbleIn .2s ease-out}.lumina-camera-placeholder{display:grid;place-items:center;width:100%;height:100%;color:rgba(255,255,255,.92)}.lumina-camera-placeholder svg{width:34%;height:34%}.lumina-camera-frame{display:block;width:100%;height:100%;border:0;background:#111827;pointer-events:none}.lumina-camera-ring{position:absolute;inset:0;border:3px solid rgba(255,255,255,.88);border-radius:inherit;box-shadow:inset 0 0 0 1px rgba(17,24,39,.12);pointer-events:none}
...
function pauseIcon() {
  return '<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M8 5h3v14H8V5Zm5 0h3v14h-3V5Z"/></svg>';
}

function playIcon() {
  return '<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M8 5v14l11-7L8 5Z"/></svg>';
}

function stopIcon() {
  return '<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M7 7h10v10H7V7Z"/></svg>';
}
...
function cameraIcon() {
  return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m16 13 5 3V8l-5 3v2Z"/><rect x="3" y="6" width="13" height="12" rx="2"/></svg>';
}

function avatarIcon() {
  return '<svg viewBox="0 0 64 64" fill="none" aria-hidden="true"><circle cx="32" cy="32" r="32" fill="rgba(255,255,255,.12)"/><circle cx="32" cy="25" r="11" fill="currentColor"/><path d="M14 56c3.2-12.5 12.1-18.5 18-18.5s14.8 6 18 18.5" fill="currentColor"/></svg>';
}
